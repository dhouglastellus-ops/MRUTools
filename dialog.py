...
class MRUDockWidget(QDockWidget):
    ...
    def refresh_summary(self) -> None:
        ...
        for feature in layer.getFeatures():
            value = feature[mru_field]
            if value is None:
                text_value = ""
            else:
                text_value = str(value).strip()

            if feature.id() in selected_ids_set:
        ...
        if origin_values:
            total_text = "; ".join(f"{value} ({counts.get(value, 0)})" for value in origin_values)
        else:
            total_text = "0"
        ...
        destination_count = counts.get(destination_value, 0) if destination_value else 0
        ...
        values = [str(value) for value in layer.uniqueValues(layer.fields().indexOf(mru_field)) if str(value).strip()]
        self._populate_combo_values(values)
